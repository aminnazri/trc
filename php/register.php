<?php
	...
	elseif (!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['confirm-password'])) {
			...
			if(isset($_POST['csrf_token']) && validateToken($_POST['csrf_token'])) {
				...
				if($C) {
					...
					if($res && $res->num_rows === 0) {
						...
						$id = sqlInsert($C, 'INSERT INTO users VALUES (NULL, ?, ?, ?, 0,?,?,0,0)', 'sssss', $_POST['name'], $_POST['email'], $hash, $last_name, $picture_link);
						if($id) {
